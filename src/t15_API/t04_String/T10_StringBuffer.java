...
/*
	...
	String 객체 : immutable객체로서 class가 한번 생성되면 그 값을 유지한다.
							주로 짧은 문자열 처리에 적당.
	StringBuffer객체 : mutable객체로서 크기를 변경할 수 있다. 동기화를 제공
										멀티스레드를 제공 및 사용하기에 적절하다. 가변길이문자열에 적당.
	
	StringBuilder객체 : mutable객체로서 크기를 변경할 수 있다. 동기화를 불분명. 가변길이문자열에 적당.
...
public class T10_StringBuffer {
	public static void main(String[] args) {
		...
		//StringBuffer()객체: 가변길이 클래스 기본크기는 16, 넘칠때는 상황에 따라 크기 변경
		// 값의 추가 : append, 특정위치삽입:insert, 특정위치값지우기:delete, 치환:replace
		//현재 버퍼의 크기? capacity()
